...
export default function Page() {
  ...
  let isToastDisplayed = false; // Flag to track whether a toast is currently displayed
  const handleLogin = async () => {
    try {
      ...
      if (response.status === 200) {
        if (!isToastDisplayed) { // Check if no toast is currently displayed
          isToastDisplayed = true; // Set the flag to true to indicate a toast is being displayed
          if (response.status === 200) {
            toast.success(response.data);
            setTimeout(() => {
              router.push("/");
              isToastDisplayed = false; // Reset the flag after the toast expires
  ...
  return (
    <div className='w-full h-screen  py-8 px-16 bg-gray-100'>
      <div className='w-full h-full p-4 flex bg-gray-100 rounded-[40px] justify-center '>
        <div className='w-[60%]'>
          <div className='w-full text-right pt-8 pr-20 text-sm'>Гишүүн биш үү? <a href='./Register' className='text-blue-500'>Одоо бүртгүүл</a></div>
          <div className='w-full flex flex-col items-center mt-[60px]'>
            <div className='w-[360px] flex justify-center flex-col'>
              <h1 className='text-[32px] font-semibold text-center mb-[20px]'>Дахин сайн уу!</h1>
              <p className='text-black/80 mb-8 text-center'>Таныг санагалзсан тул дахин тавтай морил!</p>
              <div> <input
              ...
              <button className='h-14 w-[320px bg-orange-500 rounded-md text-white  shadowyellow rounded-tr mt-8' onClick={handleLogin}>Нэвтрэх</button>
              <ToastContainer />
            </div>
          </div>
        </div>
